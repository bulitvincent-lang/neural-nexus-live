...
void main() {
  ...
  vR = r;
  gl_PointSize = uSize * (0.6 + heat) * (250.0 / -mv.z);
}
`;
...
void main() {
  ...
  gl_FragColor = vec4(col * (0.5 + vI * 1.4), a * a * clamp(vI, 0.0, 1.0));
}
`;
...
void main() {
  ...
  gl_FragColor = vec4(col * (1.0 + uGlow), clamp(a, 0.0, 0.95));
}
`;
...
export function SingularityOrb({ engineRef, detail = 1 }: OrbViewProps) {
  ...
  useFrame((_, delta) => {
    ...
    if (halo.current) halo.current.opacity = 0.1 + p.activityLevel * 0.3;
    if (group.current) {
